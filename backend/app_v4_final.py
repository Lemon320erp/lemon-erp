🍋 LEMON ERP - MASTER WORKFLOW PROMPT - v4.4.6
Use this prompt as base for all future versions. Edit/Add to this prompt for v4.4.7, v4.4.8 etc. Do not miss any detail.
...
BASE VERSION: v4.4.6 = Forget v4.4.4 and v4.4.5, Take v4.4.3 as base + SBUs final. This is stable working version on https://lemon-erp.onrender.com - File: backend/app_v4_final.py
...
1. PRODUCT CATEGORY MODULE - DB FILE FOR FURTHER USE:
...
Landing Page:
- Heading: Product Category Master - DB File for Further Use - v4.4.3 Unchanged
- Subtext: DB File: lemon_erp_v44_1_category.db - Table: product_category
- Form Box: background #FFFBEB border 2px solid var(--brass) #C9A86A
  - Title: Add Product Category - 1 Input Field
  - Hidden field: cat_id
  - Input: Category Name * - placeholder "Category Name e.g. Raw - Limestone, Finished - Quicklime 10-40mm"
...
3. SBUs MODULE - KILNS RENAMED TO SBUs - v4.4.6 FINAL SPEC:

DB Tables:
- sbu: id PK, sbu_name VARCHAR(100), address TEXT, created_at
- kiln_asset: id PK, sbu_id FK sbu.id, kiln_no VARCHAR(50), lining_installation_date VARCHAR(20), health_status VARCHAR(50), products_capacity TEXT JSON - list of {product_id, capacity_per_day, capacity, lining_date, lining_installation_date, health_status}
- sizing_plant_asset: id PK, sbu_id FK, plant_no VARCHAR(50), products_capacity TEXT JSON - list of {product_id, capacity_per_hour, capacity, machineries}, machineries TEXT
...
- Card text-align center padding 24px
  ...
  - P: Module name Kilns renamed to SBUs - Landing page heading Strategic Business Units - 11px gray #666 text-align center margin 0 0 18px
  - Button: Add New SBU - padding 14px 36px font 15px weight 800 btn-y lemon - onclick openAddSBU()
  - P: b-Add New SBU Button Below Heading - Popup with X closing option on top right - 10px color #888 margin-top 12px
- Div id sbuList: Loading SBUs...

Popup: id sbuModal - class modal hidden - onclick if event.target===this closeAddSBU() - modal-content max-width 1000px max-height 94vh flex column - Header: Add SBU - Strategic Business Units - Popup with X + close-x button x - Body: hidden sbu_id + form-box SBU Details background #FFFBEB border 2px brass
  ...
  - Asset Section 1: class asset-section background white border 1.5px solid var(--line) border-radius 12px padding 14px margin 14px 0
    - Header: h4 🔥 Kilns - Add Kiln Button → Add new line *Kiln No. *Products and Capacity *Add Product Button *Delete button - Button Add Kiln onclick addKilnField()
    - P: When clicked Add Kiln - Add new line of input field as *Kiln No. *Products and Capacity *Add Product Button *Delete button for deleting this line if not required. - 10px gray
    - Container: id kilnsContainer - P No kilns - Click Add Kiln Button → Add new line
  - Asset Section 2: ⚙️ Sizing Plants - Add Sizing Plant Button → *Plant No. *Products and Capacity *Add Product Button - Button Add Sizing Plant - P When clicked on add Sizing Plant Button - Add new line of input field as: *Plant No. *Products and Capacity *Add Product Button. - Container sizingContainer
  - Asset Section 3: 💧 Hydration Plants - Add Hydration Plant Button → *Plant No. *Products and Capacity *Add Product Button - Button Add Hydration Plant - Container hydrationContainer No hydration plants
  - Asset Section 4: 📦 Stock Yards - Add Stock Yard Button → *Yard Name *Add Yard Items - Button Add Stock Yard btn-y - P Add Stock Yard Button - when clicked on add Button - Add new line of input fields as *Yard Name *Add Yard Items - when clicked on add Button - Add new line of input fields ~dropdown selection field (take data of products from all category, Opening stock of each product stored in this yard. - 10px gray - Container yardsContainer No stock yards
- Footer: Save SBU - Strategic Business Units btn-g flex 1 padding 14px font 13px onclick saveSBU() + Cancel btn-w

Kiln Field: function addKilnField(data=null)
...
- HTML: div id=${id} class kiln-line background #FFFBEB border 1.5px solid var(--brass) border-radius 10px padding 12px margin 10px 0
  - Top row: flex justify space-between wrap - b Kiln Line - *Kiln No. *Products and Capacity *Add Product Button *Delete button for deleting line - Buttons: Add Product onclick addKilnProduct('${id}') btn-b background #E8F0FE + Delete Kiln btn-r #C5221F onclick remove
  - Row: *Kiln No. - e.g. Kiln 1, K-01 - input class k_no placeholder "Kiln No. - e.g. Kiln 1, K-01" value data.kiln_no
  - Products Section: margin-top 8px padding 10px background white border-radius 8px border 1px solid var(--line) - b *Products and Capacity - when clicked on add product Button - Add new line of input fields as Inset ~Product name (selected from Finished Product List) Capacity/Day ~Lining Installation Date ~Health Status ~delete button - 11px - Container kiln-products-container - If data.products_capacity map renderKilnProductLine else P No products - Click Add Product → Inset Product name, Capacity/Day, Lining Date, Health Status, Delete - 10px gray

Kiln Product Line: function renderKilnProductLine(pc)
...
- div id product-line background white border 1px dashed var(--brass) border-radius 8px padding 10px margin 8px 0 margin-left 12px border-left 4px solid var(--brass)
  - Row align end: Product name (selected from Finished Product List) - select class kp_product options from finishedProducts (filter category includes finish/quicklime/CaO, else all) - getFinishedProductOptions(product_id) - Capacity/Day MT/day e.g. 15 - input class kp_capacity type number placeholder "Capacity/Day - MT/day e.g. 15" value capacity_per_day - Lining Installation Date Date picker - input class kp_lining type date value lining_date - Health Status Good, Needs Repair, Critical, New - select class kp_health options Good/Needs Repair/Critical/New selected health_status - Delete button btn-r margin-top 18px onclick remove
...
JS Helpers:
- loadAllProductsForSBU(): fetch /api/products -> window.allProducts, window.finishedProducts filter
...
Save SBU: async saveSBU()
- sbuName = sbu_name trim - alert if empty
- kilns = querySelectorAll #kilnsContainer > div[id^="kiln_"] - for each div: products_capacity = querySelectorAll .kiln-products-container > div[id^="kprod_"] - for each pdiv: pid kp_product value, cap kp_capacity, lining kp_lining, health kp_health - if pid push {product_id parseInt, capacity_per_day parseFloat, capacity, lining_date, lining_installation_date, health_status}
- kilns push {kiln_no k_no value, products_capacity}
...
Load SBUs: async loadSBUs()
- fetch /api/sbus - sbus array
- sbuCountDash = length
- if length 0: sbuList innerHTML div center padding 30px P No SBUs - Masters empty - Strategic Business Units - Forget v4.4.4 and v4.4.5 - Take v4.4.3 as base + P Landing page heading Strategic Business Units - Add New SBU Button Below Heading - Popup with X - SBU Name, Address, Add Kiln Button → Kiln No., Products and Capacity Add Product Button Delete button Inset Product name Finished List Capacity/Day Lining Date Health Status Delete button + Sizing Plant No. Products and Capacity Add Product Product name Capacity/hour Machineries + Hydration Plant No. Products and Capacity Add Product Product name Capacity/hour Machineries + Stock Yard Yard Name Add Yard Items Product from all category Opening stock + Button Add First SBU
- else h = '' for s in sbus: kilnBadge = s.kilns length etc - h+= card sbu-card flex justify space-between wrap gap 10px - div h3 16px bi-building s.sbu_name + p 11px gray geo-alt address + p 10px margin-top 6px badges brass 3 Kilns etc + c- SBU card with SBU Name + Address + Badges example + div buttons Edit onclick editSBU(s.id) + Delete delSBU + detail-grid
  - detail-grid: div b 11px Kilns list with No, Capacity, Lining Date, Health Status badge, Products - d-Detailed grid: - map k => div 11px background #FFFBEB padding 8px margin 6px 0 border-radius 8px border 1.5px brass b kiln_no + map products_capacity => span display block background white padding 4px 6px border-radius 4px margin 3px 0 border 1px line b product_name - capacity MT/day - Product name Finished List Capacity/Day br Lining: lining_date Date picker | Health: badge ok/crit/warn health_status - Health Status Good/Needs Repair/Critical/New + No products - similarly sizing, hydration, yards - with b 11px headings
  - p 10px gray margin-top 10px c- SBU card with SBU Name + Address + Badges: kilnBadge etc - d-Detailed grid: Kilns list... / Edit / Delete buttons for each SBU - Base v4.4.3 Products refined kept
- sbuList innerHTML = h

Edit SBU: async editSBU(id)
- fetch /api/sbus/id - s
- sbu_id = s.id, sbu_name = s.sbu_name, sbu_address = s.address
- openAddSBU()
- setTimeout 600ms: kilnsContainer innerHTML='', sizingContainer='', hydrationContainer='', yardsContainer='' - forEach k addKilnField(k), sp addSizingField(sp), hp addHydrationField(hp), y addYardField(y)
...
- POST /api/sbus - body sbu_name, address, kilns array kiln_no, products_capacity, sizing_plants plant_no, products_capacity, machineries, hydration_plants same, stock_yards yard_name, yard_items - create SBU flush, create KilnAsset sbu_id, kiln_no, lining_installation_date '', health_status 'Good', products_capacity json dumps, SizingPlantAsset, HydrationPlantAsset, StockYardAsset - commit - return id, sbu_name
...
- Dash: id dash - card Dash - v4.4.6 etc - kpi-grid 4 cards Total Value totalVal Rs Lakh, SBUs sbuCountDash, Products prodCountDash, Categories catCountDash - card v4.4.3 Base + SBUs Final description - card Alerts alerts - loadDash() fetch inventory combined total_value_lakh, alerts, mo total, product_categories count, products count, sbus count
...
- Backend Flask: app Flask, secret_key lemon-erp-v44-6-sbus-final-base-v443, SQLALCHEMY_DATABASE_URI sqlite:///lemon_erp_v44_1_category.db, TRACK_MODIFICATIONS False, db SQLAlchemy - All models as listed - db.create_all() in app_context - generate_product_code function - All API routes as defined - health route /api/health status LIVE version v4.4.6 etc db_file lemon_erp_v44_1_category.db url https://lemon-erp.onrender.com
...
- Keep code same as v4.4.6 for all modules unless explicitly asked to change that module.
- When user says "keep everything unchanged in v4.4.6 except these changes in SBUs" - only change SBUs module - Do not change Product Category, Products, Stock, Make, Buy, Sell, Vendors, Customers, Pack, QR, Cost, Mobile, Dash, CSS, other DB tables.
- When user says "while adding kilns lining and health status is asked repeatedly for all product instead it is for the kiln put under kiln name asked once when add product dont club lining and health" - Implement: In addKilnField, have Row with Kiln No + Lining Installation Date date input + Health Status select - asked once per kiln - In renderKilnProductLine, only Product name select + Capacity/Day number - remove lining and health from product line - In saveSBU, collect lining and health from kiln div (k_lining, k_health) not from product div - Store in kiln_asset lining_installation_date and health_status columns - products_capacity JSON only product_id and capacity_per_day
- When user says "when edit any sbu it create new sbu and name and address erased fix bug and keep edit button" - Fix: editSBU currently does set sbu_id, sbu_name, sbu_address then openAddSBU() which clears them - Bug - Fix by calling openAddSBU() first then setTimeout to set id, name, address and populate containers - Keep edit button
- When user says "Provide Option to create duplicate of SBU so that we need not fill each details again and again" - Add function duplicateSBU(id) - fetch SBU, create payload with sbu_name + ' - Copy', same address, kilns map kiln_no + ' - Copy', same lining, health, products_capacity product_id capacity, sizing plants plant_no + ' - Copy', etc, stock yards yard_name + ' - Copy', yard_items product_id opening_stock - POST /api/sbus - Alert Duplicated - loadSBUs() - Add button Duplicate btn-o in SBU card next to Edit Delete
- When user says "remove text line Module name Kilns renamed to SBUs - Landing page heading Strategic Business Units after Heading and line b-Add New SBU Button Below Heading - Popup with X closing option on top right after add new SBU button" - Remove those two P tags in sbus tabcontent card - Keep only H1 Strategic Business Units and Button Add New SBU
- When user says "arrange data of sbus in tabular format so as to look clean" - Change loadSBUs detailed grid to tabular: For each SBU, after header, create divs with border 1.5px solid var(--line) border-radius 10px overflow hidden margin 10px 0 - Header div background var(--green) color white padding 8px 10px font 800 11px - Table with th Kiln No, Lining Date, Health, Products + Capacity/Day etc - Rows map - Clean tabular
